...
```{r}
hyp_samples_lf <-hyp_samples %>% filter(trt == "lowfat")
hyp_countdata_lf <- dplyr::select(hyp_countdata, contains("lowfat"))

hyp_samples_sd <-hyp_samples %>% filter(trt == "standard")
hyp_countdata_sd <- dplyr::select(hyp_countdata, contains("standard"))

ddssex <- DESeqDataSetFromMatrix(countData = hyp_countdata_sd,
                                      colData = hyp_samples_sd,
                                       design = ~ sex)
...
restrt_hyp <-restrt
write.csv(restrt_hyp, file="results/GOresults/hyp/restrt_hyp.csv")
...
gostres_hyp_up <- gost(query = rownames(up_hyp), 
...
gostres_hyp_down <- gost(query = rownames(down_hyp), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.05, correction_method = "g_SCS", 
...
write.xlsx(setNames(as.list(lapply(go_hyp, data.frame)), names(go_hyp)), file="go_hyp_trt.xlsx")
go_hyp <- go_hyp %>% reduce(full_join, by=c("V1","V2"))
```

```{r}
up_hyp <- filter(as.data.frame(ressex), log2FoldChange > 0, padj < 0.05)
gostres_hyp_up <- gost(query = rownames(up_hyp), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
down_hyp <- filter(as.data.frame(ressex), log2FoldChange < 0, padj < 0.05)
gostres_hyp_down <- gost(query = rownames(down_hyp), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
go_hyp <- list(go_hyp_down,go_hyp_up)
write.xlsx(setNames(as.list(lapply(go_hyp, data.frame)), names(go_hyp)), file="go_hyp_sex.xlsx")
go_hyp <- go_hyp %>% reduce(full_join, by=c("V1","V2"))
```
