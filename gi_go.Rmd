...
mcols(gi_dds_trt) <- cbind(mcols(gi_dds_trt), hgnc_symbol)
res_object$hgnc_symbol <- rowData(gi_dds_trt)$hgnc_symbol
...
gi_volcano2 <- EnhancedVolcano(res_object,
                ...
                borderWidth = 1.5,
                borderColour = 'black')
ggsave(paste0("figures/gi/gi_volcano2_", verson_num, ".png"), width = 8, height = 8, dpi = 600)
gi_volcano2
...
vst_macrophage <- varianceStabilizingTransformation(gi_dds_trt)

bg_ids <- rownames(gi_dds_trt)[rowSums(counts(gi_dds_trt)) > 0]
...
enhance_table <- enhance_table(res_enrich,
                   ...
                   chars_limit = 60)
enhance_table
ggsave(paste0("GO/gi/gi_enhance_table_", verson_num, ".png"), width = 8, height =15, dpi = 600)
...
volcano_plot
ggsave(paste0("GO/gi/gi_volcano_plot_", verson_num, ".png"), width = 8, height = 4.6, dpi = 600)
...
mds <- gs_mds(res_enrich,
       ...
       plot_title = NULL) 
mds
ggsave(paste0("GO/gi/gi_mds_", verson_num, ".png"), width = 8, height = 4.6, dpi = 600)
...
#geneset_volcano <- signature_volcano(res_de = gi_res_trt,
#                  res_enrich = res_enrich,
#                  annotation_obj = anno_df,
...
#                  color = "#1a81c2"
#)
#ggsave("GO/gi_geneset_volcano.png", width = 8, height = 4.6, dpi = 600)
#geneset_volcano
}
```
...
gi_res_trt <- results(gi_dds_trt, alpha = 0.05)
gi_res_trt$symbol <- mcols(gi_res_trt)$symbol

# how many are "significant"?
table(gi_res_trt[,"padj"] < 0.05)
summary(gi_res_trt)


#BiocManager::install("vsn")
library("vsn")
vsd <- varianceStabilizingTransformation(gi_dds_trt, blind = FALSE)
...
ggplotly(pca)
ggsave("figures/gi_PCA.png", width = 8, height = 4.6, dpi = 600)
...
gi_dds_trt <- estimateSizeFactors(gi_dds_trt)
normalized_counts <- counts(gi_dds_trt, normalized=TRUE)
...
```{r}
gi_samples_lf <-gi_samples %>% filter(trt == "lowfat")
gi_countdata_lf <- dplyr::select(gi_countdata, contains("lowfat"))

gi_samples_sd <-gi_samples %>% filter(trt == "standard")
gi_countdata_sd <- dplyr::select(gi_countdata, contains("standard"))


ddssex <- DESeqDataSetFromMatrix(countData = gi_countdata_sd,
                                       colData = gi_samples_sd,
                                       design = ~ sex)
...
```{r}
ddstrt <- DESeqDataSetFromMatrix(countData = gi_countdata,
                                       colData = gi_samples,
                                       design = ~ trt + sex)
...
#rename transcript rows to ensid names
#rownames(ddstrt)=annos$ensid[match(names(ddstrt),annos$gene)]s
#remove the NAs (those transcripts dont have a ensid name)
...
restrt_gi <-restrt
write.csv(restrt_gi, file="results/GOresults/gi/restrt_gi.csv")
...
go trt
```{r}
up_gi <- filter(as.data.frame(restrt_gi), log2FoldChange > 0, padj < 0.05)
gostres_gi_up <- gost(query = rownames(up_gi), 
...
go_gi_up <- as.data.frame(cbind(gostres_gi_up$result$term_id,gostres_gi_up$result$term_name, gostres_gi_up$result$intersection_size, gostres_gi_up$result$p_value))
go_gi_up$tissue <- "gi"
go_gi_up$DE <- "up"
go_gi_up <- dplyr::filter(go_gi_up, grepl('GO', V1))
go_gi_up <- top_n(go_gi_up, 20, V4)

down_gi <- filter(as.data.frame(restrt_gi), log2FoldChange < 0, padj < 0.05)
gostres_gi_down <- gost(query = rownames(down_gi), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.05, correction_method = "g_SCS", 
...
go_gi_down <- as.data.frame(cbind(gostres_gi_down$result$term_id,gostres_gi_down$result$term_name, gostres_gi_down$result$intersection_size, gostres_gi_down$result$p_value))
go_gi_down$tissue <- "gi"
go_gi_down$DE <- "down"
go_gi_down <- dplyr::filter(go_gi_down, grepl('GO', V1))
go_gi_down <- top_n(go_gi_down, 20, V4)
go_gi <- list(go_gi_down,go_gi_up)
write.xlsx(setNames(as.list(lapply(go_gi, data.frame)), names(go_gi)), file="go_gi_restrt.xlsx")
go_gi <- go_gi %>% reduce(full_join, by=c("V1","V2"))
```

```{r}
up_gi <- filter(as.data.frame(ressex_gi_sd), log2FoldChange > 0, padj < 0.05)
gostres_gi_up <- gost(query = rownames(up_gi), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
...
go_gi_up <- as.data.frame(cbind(gostres_gi_up$result$term_id,gostres_gi_up$result$term_name, gostres_gi_up$result$term_size, gostres_gi_up$result$p_value))
go_gi_up$tissue <- "gi"
go_gi_up$DE <- "up"
go_gi_up <- dplyr::filter(go_gi_up, grepl('GO', V1))
go_gi_up <- top_n(go_gi_up, 20, V4)
...
down_gi <- filter(as.data.frame(ressex_gi_sd), log2FoldChange < 0, padj < 0.05)
gostres_gi_down <- gost(query = rownames(down_gi), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
go_gi_down <- as.data.frame(cbind(gostres_gi_down$result$term_id,gostres_gi_down$result$term_name, gostres_gi_down$result$term_size, gostres_gi_down$result$p_value))
go_gi_down$tissue <- "gi"
go_gi_down$DE <- "down"
go_gi_down <- dplyr::filter(go_gi_down, grepl('GO', V1))
go_gi_down <- top_n(go_gi_down, 20, V4)
...
go_gi <- list(go_gi_down,go_gi_up)
write.xlsx(setNames(as.list(lapply(go_gi, data.frame)), names(go_gi)), file="go_gi_ressex_sd.xlsx")
go_gi <- go_gi %>% reduce(full_join, by=c("V1","V2"))
```
