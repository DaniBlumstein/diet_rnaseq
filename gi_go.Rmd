...
```{r}
ddssex <- DESeqDataSetFromMatrix(countData = gi_countdata,
                                       colData = gi_samples,
                                       design = ~ trt + sex)
...
write.csv(restrt, file="results/GOresults/gi/gi_de_GO.csv")
...
res_enrich <- shake_topGOtableResult(topgoDE_macrophage_IFNg_vs_naive)
res_enrich_gi <- get_aggrscores(res_enrich = res_enrich,
                                        res_de = restrt,
                                        annotation_obj = anno_df,
                                        aggrfun = mean)

#GeneTonic(dds = gi_dds_trt,
#          res_de = restrt,
#          res_enrich = res_enrich_gi,
#          annotation_obj = anno_df,
#          project_id = "myFirstGeneTonic")
#

#rename transcript rows to ensid names
rownames(gi_dds_trt)=annos$gene[match(names(gi_dds_trt),annos$ensid)]

gene_list <- c("MFSD2A","F2")
plist <- c()
for (i in gene_list) {
  
plist[[i]] <- gene_plot(gi_dds_trt,
          gene = c(i),
          intgroup = "trt",
          assay = "counts",
          annotation_obj = anno_df,
...
gi_DE_plots <- gridExtra::grid.arrange(grobs = plist, ncol=1)

df <- get_expression_values(dds = gi_dds_trt, gene = "MFSD2A", intgroup = "trt", 
        assay = "counts")

ggplot(df,aes(x=trt,y=log10(exp_value), color = trt))+
  geom_boxplot()+
  geom_jitter()+
  #ylim(100,6000)+
  #title("MFSD2A - GI")+
...
```{r}
up_gi <- filter(as.data.frame(restrt), log2FoldChange > 0, padj < 0.05)
gostres_gi_up <- gost(query = up_gi$hgnc_symbol, 
...
down_gi <- filter(as.data.frame(restrt), log2FoldChange < 0, padj < 0.05)
gostres_gi_down <- gost(query = down_gi$hgnc_symbol, 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.05, correction_method = "g_SCS", 
