...
mcols(kid_dds_trt) <- cbind(mcols(kid_dds_trt), hgnc_symbol)
res_object$hgnc_symbol <- rowData(kid_dds_trt)$hgnc_symbol
...
kid_volcano2 <- EnhancedVolcano(res_object,
                ...
                borderWidth = 1.5,
                borderColour = 'black')
ggsave(paste0("figures/kid/kid_volcano2_", verson_num, ".png"), width = 8, height = 8, dpi = 600)
kid_volcano2
...
bg_ids <- rownames(kid_dds_trt)[rowSums(counts(kid_dds_trt)) > 0]
...
enhance_table <- enhance_table(res_enrich,
                   ...
                   chars_limit = 60)
enhance_table
ggsave(paste0("GO/kid/kid_enhance_table_", verson_num, ".png"), width = 8, height =15, dpi = 600)
...
volcano_plot
ggsave(paste0("GO/kid/kid_volcano_plot_", verson_num, ".png"), width = 8, height = 4.6, dpi = 600)
...
mds <- gs_mds(res_enrich,
       ...
       plot_title = NULL) 
mds
ggsave(paste0("GO/kid/kid_mds_", verson_num, ".png"), width = 8, height = 4.6, dpi = 600)
...
kid_res_trt <- results(kid_dds_trt, alpha = 0.05)
kid_res_trt$symbol <- mcols(kid_res_trt)$symbol

# how many are "significant"?
table(kid_res_trt[,"padj"] < 0.05)
summary(kid_res_trt)


#BiocManager::install("vsn")
library("vsn")
vsd <- varianceStabilizingTransformation(kid_dds_trt, blind = FALSE)
...
kid_dds_trt <- estimateSizeFactors(kid_dds_trt)
normalized_counts <- counts(kid_dds_trt, normalized=TRUE)
...
```{r}
kid_samples_lf <-kid_samples %>% filter(trt == "lowfat")
kid_countdata_lf <- dplyr::select(kid_countdata, contains("lowfat"))

kid_samples_sd <-kid_samples %>% filter(trt == "standard")
kid_countdata_sd <- dplyr::select(kid_countdata, contains("standard"))

ddssex <- DESeqDataSetFromMatrix(countData = kid_countdata_sd,
                                       colData = kid_samples_sd,
                                       design = ~ sex)
...
ressex_kid_sd <-ressex
```


best model for downstream things right now
```{r}
ddstrt <- DESeqDataSetFromMatrix(countData = kid_countdata,
                                       colData = kid_samples,
                                       design = ~ trt + sex)
...
restrt_kid <-restrt
summary(restrt_kid)
write.csv(restrt_kid, file="results/GOresults/kid/restrt_kid.csv")
...
pca <- ggplot(pcaData, aes(x = PC1, y = PC2, color = trt, shape = sex, name=name)) +
  ...
  guides(shape = guide_legend(order = 1),color = guide_legend(order = 2)) 
ggplotly(pca)
ggsave("figures/kid_PCA.png", width = 8, height = 4.6, dpi = 600)
```
...
```{r}
library(gprofiler2)
up_kid <- filter(as.data.frame(restrt), log2FoldChange > 0, padj < 0.05)
gostres_kid_up <- gost(query = rownames(up_kid), 
...
go_kid_up <- as.data.frame(cbind(gostres_kid_up$result$term_id,gostres_kid_up$result$term_name, gostres_kid_up$result$term_size, gostres_kid_up$result$p_value))
go_kid_up$tissue <- "kid"
go_kid_up$DE <- "up"
go_kid_up <- dplyr::filter(go_kid_up, grepl('GO', V1))
go_kid_up <- top_n(go_kid_up, 20, V4)

down_kid <- filter(as.data.frame(restrt), log2FoldChange < 0, padj < 0.05)
gostres_kid_down <- gost(query = rownames(down_kid), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.05, correction_method = "g_SCS", 
...
go_kid_down <- as.data.frame(cbind(gostres_kid_down$result$term_id,gostres_kid_down$result$term_name, gostres_kid_down$result$term_size, gostres_kid_down$result$p_value))
go_kid_down$tissue <- "kid"
go_kid_down$DE <- "down"
go_kid_down <- dplyr::filter(go_kid_down, grepl('GO', V1))
go_kid_down <- top_n(go_kid_down, 20, V4)

go_kid <- list(go_kid_down,go_kid_up)
write.xlsx(setNames(as.list(lapply(go_kid, data.frame)), names(go_kid)), file="go_kid_restrt.xlsx")
go_kid <- go_kid %>% reduce(full_join, by=c("V1","V2"))
```


```{r}
up_kid <- filter(as.data.frame(ressex_kid_sd), log2FoldChange > 0, padj < 0.05)
gostres_kid_up <- gost(query = rownames(up_kid), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
down_kid <- filter(as.data.frame(ressex_kid_sd), log2FoldChange < 0, padj < 0.05)
gostres_kid_down <- gost(query = rownames(down_kid), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
go_kid <- list(go_kid_down,go_kid_up)
write.xlsx(setNames(as.list(lapply(go_kid, data.frame)), names(go_kid)), file="go_kid_ressex _sd.xlsx")
go_kid <- go_kid %>% reduce(full_join, by=c("V1","V2"))
```
