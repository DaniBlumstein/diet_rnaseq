...
```{r}
liv_samples_lf <-liv_samples %>% filter(trt == "lowfat")
liv_countdata_lf <- dplyr::select(liv_countdata, contains("lowfat"))

liv_samples_sd <-liv_samples %>% filter(trt == "standard")
liv_countdata_sd <- dplyr::select(liv_countdata, contains("standard"))

ddssex <- DESeqDataSetFromMatrix(countData = liv_countdata_sd,
                                       colData = liv_samples_sd,
                                       design = ~ sex)
...
restrt_liv <-restrt
write.csv(restrt_liv, file="results/GOresults/liv/restrt_liv.csv")
...
gostres_liv_up <- gost(query = rownames(up_liv), 
...
gostres_liv_down <- gost(query = rownames(down_liv), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.05, correction_method = "g_SCS", 
...
write.xlsx(setNames(as.list(lapply(go_liv, data.frame)), names(go_liv)), file="go_liv_trt.xlsx")
go_liv <- go_liv %>% reduce(full_join, by=c("V1","V2"))
```

```{r}
up_liv <- filter(as.data.frame(ressex), log2FoldChange > 0, padj < 0.05)
gostres_liv_up <- gost(query = rownames(up_liv), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
down_liv <- filter(as.data.frame(ressex), log2FoldChange < 0, padj < 0.05)
gostres_liv_down <- gost(query = rownames(down_liv), 
                organism = "hsapiens", ordered_query = FALSE, 
                multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
...
go_liv <- list(go_liv_down,go_liv_up)
write.xlsx(setNames(as.list(lapply(go_liv, data.frame)), names(go_liv)), file="go_liv_sex.xlsx")
go_liv <- go_liv %>% reduce(full_join, by=c("V1","V2"))
```
